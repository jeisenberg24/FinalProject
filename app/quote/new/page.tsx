...
export default function NewQuotePage() {
  ...
  const handleSave = async () => {
    if (!isLoggedIn || !user || !quoteResult) {
    ...
    try {
      ...
      // Calculate base price for storage
      const basePrice = marketRate * laborHours;
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          market_rate: basePrice,
          market_demand: marketDemand === "Low" ? "Low" : marketDemand === "High" ? "High" : marketDemand === "Peak" ? "High" : "Medium",
          is_emergency: isEmergency,
          location: location || "Location",
          complexity: complexity === "Expert" ? "Complex" : complexity,
          materials_cost: materialsCost > 0 ? materialsCost : undefined,
          time_of_day: timeOfDay === "Evening/Weekend" ? "evening" : undefined,
          seasonal_factor: seasonalFactor === "Off-season" ? "Off-peak" : seasonalFactor === "Busy season" ? "Peak" : "Normal",
          competitor_pricing: competitorPrice,
          experience_level: experienceLevel === "Entry" ? "Beginner" : experienceLevel === "Pro" ? "Expert" : experienceLevel === "Expert" ? "Expert" : "Intermediate",
          equipment_requirements: equipmentCost > 0 ? (equipmentCost > 100 ? "Heavy-duty" : "Specialized") : "Standard",
          calculated_price: quoteResult.calculatedPrice,
          price_range_min: quoteResult.priceRange.min,
          price_range_max: quoteResult.priceRange.max,
          price_breakdown: quoteResult.breakdown,
          quote_validity_days: quoteResult.validityDays,
        })
        .select()
        .single();
