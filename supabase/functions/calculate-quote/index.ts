...
// Pricing multipliers (same as client-side)
const MARKET_DEMAND_MULTIPLIERS = { High: 1.15, Medium: 1.0, Low: 0.9 };
const COMPLEXITY_MULTIPLIERS = { Simple: 0.9, Moderate: 1.0, Complex: 1.2 };
const SEASONAL_MULTIPLIERS = { Peak: 1.15, Normal: 1.0, "Off-peak": 0.9 };
const EXPERIENCE_MULTIPLIERS = { Beginner: 0.95, Intermediate: 1.0, Expert: 1.1 };
const EQUIPMENT_COSTS = { Standard: 0, Specialized: 50, "Heavy-duty": 150 };
const TRAVEL_RATE_PER_MILE = 0.65;
const EMERGENCY_BASE_PREMIUM = 0.5;
const EMERGENCY_AFTER_HOURS_PREMIUM = 0.25;
const CONFIDENCE_INTERVAL = 0.1;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Validate input
    if (!input.marketRate || !input.serviceType || !input.location) {
